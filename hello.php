...
if ( ! class_exists( 'avia_masonry' ) )
{
	class avia_masonry
	{
		...
		public function html()
		{
			...
			$output .= "<div id='{$id_el}' class='atin av-masonry {$custom_class} noHover av-{$size}-size av-{$this->atts['gap']}-gap av-hover-overlay-{$this->atts['overlay_fx']} av-masonry-animation-{$this->atts['animation']} av-masonry-col-{$this->atts['columns']} av-caption-{$this->atts['caption_display']} av-caption-style-{$this->atts['caption_styling']} {$this->atts['container_class']} {$orientation} {$av_display_classes} {$av_column_classes}' {$style} >";
			...
			foreach( $this->loop as $index => $entry )
			{
				...
				if(!empty($attachment))
				{
					...
					//custom code by atin
					$custom_fild=get_attachment_fields_to_edit($thumb_ID, $errors = null);
					echo json_encode($custom_fild['custom1']['value']);
					if(isset($attachment[0]))
					{
						if($size == 'flex')  
						{
							...
							$img_html  = '<img data-name="'.$custom_fild['custom1']['value'].'" src="'.$attachment[0].'" title="'.$title.'" alt="'.$alt.'" />';
							$outer_title = '';
						}
		...
		protected function load_more_button()
		{
			...
			$output  = '';
			$output .= 		"<div class='loadMoreBtn'><a class='av-masonry-pagination av-masonry-load-more' href='#load-more' {$data_string}>" . __( 'Load more', 'avia_framework' ) . "</a></div>";
